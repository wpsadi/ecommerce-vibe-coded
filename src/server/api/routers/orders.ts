				...
				if (search) {
					conditions.push(
						or(
							...
							sql`CAST(${orders.shippingAddress}->>'firstName' AS TEXT) ILIKE ${`%${search}%`}`,
							sql`CAST(${orders.shippingAddress}->>'lastName' AS TEXT) ILIKE ${`%${search}%`}`,
							// Add search by user email if needed, requires joining users table
						),
					);
				}
