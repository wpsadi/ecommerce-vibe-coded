...
export const useProduct = (id: string) => {
    return api.products.getById.useQuery({ id }, {
        enabled: !!id,
...
export const useProductBySlug = (slug: string) => {
    return api.products.getBySlug.useQuery({ slug }, {
        enabled: !!slug,
...
export const useProductImages = (productId: string) => {
    return api.products.getImages.useQuery({ productId }, {
        enabled: !!productId,
...
export const useRelatedProducts = (categoryId: string, currentProductId: string) => {
    return api.products.getAll.useQuery(
        { categoryId, limit: 4 },
        {
            enabled: !!categoryId,
            select: (data) => data?.filter(product => product.id !== currentProductId) || [],
            staleTime: 10 * 60 * 1000,
        }
    )
}
...
export const useLowStockProducts = (threshold?: number) => {
    return api.products.getLowStock.useQuery({ threshold }, {
        staleTime: 5 * 60 * 1000,
    })
}
...
export const useCategory = (id: string) => {
    return api.categories.getById.useQuery({ id }, {
        enabled: !!id,
        staleTime: 15 * 60 * 1000,
    })
}
...
export const useCategoryBySlug = (slug: string) => {
    return api.categories.getBySlug.useQuery({ slug }, {
        enabled: !!slug,
        staleTime: 15 * 60 * 1000,
    })
}
    ...
    return api.cart.addItem.useMutation({
        ...
        onMutate: async (newItem) => {
            ...
            utils.cart.getItems.setData(undefined, (old) => {
                if (!old) return old
                const existingItem = old.find(item => item.product.id === newItem.productId)
                if (existingItem) {
                    return old.map(item =>
...
export const useAddToWishlist = () => {
    ...
    return api.wishlist.addItem.useMutation({
        ...
        onMutate: async (variables) => {
            ...
            await utils.wishlist.isInWishlist.cancel({ productId: variables.productId })

            const previousCount = utils.wishlist.getCount.getData()
            const previousWishlistStatus = utils.wishlist.isInWishlist.getData({ productId: variables.productId })
...
export const useOrder = (id: string) => {
    return api.orders.getById.useQuery({ id }, {
        enabled: !!id,
...
export const useAddresses = (type?: "shipping" | "billing") => {
    return api.users.getAddresses.useQuery({ type }, {
        staleTime: 5 * 60 * 1000,
    })
}

export const useAddress = (id: string) => {
    return api.users.getAddressById.useQuery({ id }, {
        enabled: !!id,
        staleTime: 5 * 60 * 1000,
    })
}
